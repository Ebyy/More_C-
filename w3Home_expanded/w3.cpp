...
int main(int argc, char* argv[]) {
	...
	do {
		...
		if (index) {
			...
			//
            SummableLVPair<std::string, std::string> tempRefObject(label, reference);

			references += tempRefObject;
		}
	} while (index);
	index.clear();
	...
	do {
		...
		if (sales) {
			...
			//
			SummableLVPair<std::string, int> tempTicketObject(ticketType, number);
			ticketSales += tempTicketObject;
		}
	} while (sales);
	sales.clear();
